...
(function () {
  ...
  /* ---- De un carrusel por categoría a varios ----
     ...
     guarda una lista en `info.carousels`, y cada uno lleva su propia
     posición entre las imágenes del menú.

     Lo que ya está guardado sigue funcionando: si aparece el formato viejo
     se lee como una lista de uno. No hay que migrar nada a mano, y el
     content.json que está en vivo no se toca hasta que se guarde de nuevo
     desde el panel. */

  /** La lista de carruseles de una categoría, en cualquiera de los dos
   *  formatos, sin filtrar. */
  function carouselList(info) {
    if (!info) return [];
    if (Array.isArray(info.carousels)) return info.carousels;
    // formato viejo: un objeto suelto
    if (info.carousel && typeof info.carousel === "object") return [info.carousel];
    return [];
  }

  /** ¿Este carrusel se puede mostrar? Activo y con al menos una imagen
   *  activa; si no, null y no se dibuja nada — sin dejar hueco. */
  function usableCarousel(c) {
  ...
  /** Los que de verdad se van a dibujar, ordenados por su posición.
   *
   *  El orden importa: dos carruseles en la misma posición tienen que salir
   *  siempre en el mismo orden, y no según cómo quedaron guardados. Se
   *  desempata por su lugar en la lista. */
  function activeCarousels(info) {
    var lista = carouselList(info).map(function (c, i) {
      var u = usableCarousel(c);
      return u ? { cfg: u, pos: Math.max(0, Number(u.position) || 0), orden: i } : null;
    }).filter(Boolean);
    lista.sort(function (a, b) { return a.pos - b.pos || a.orden - b.orden; });
    return lista.map(function (x) { return x.cfg; });
  }

  /** Compatibilidad: el primero que se pueda mostrar, o null. */
  function activeCarouselFor(info) {
    return activeCarousels(info)[0] || null;
  }
  ...
  function initCategoryCarousel(root, cfg) {
    ...
    /* Si el cliente elige una foto a mano, el carrusel se queda ahí: no se lo
       vuelve a mover por debajo mientras la está mirando. Se reanuda cuando
       vuelve a desplazar la página, que es la señal de que ya siguió. */
    ...
    function goTo(i, fromUser) {
      ...
      paint();
      if (fromUser) { pickedByUser = true; pickedAt = Date.now(); stop(); }
    }
    function stop() { clearInterval(timer); timer = null; }
    root.__ccStop = stop; // permite apagarlo desde fuera (vista previa del admin)
    function restart() {
      stop();
      if (pickedByUser || cfg.autoplay === false || total < 2) return;
    ...
    if (total > 1) {
      ...
      /* Desplazar la página es la señal de "ya seguí": si el cliente había
         elegido una foto a mano, el movimiento automático vuelve a arrancar.
         El margen de 600ms evita que el propio gesto de deslizar la foto
         (que arrastra un poco la página) lo reanude al instante. */
      window.addEventListener("scroll", function () {
        if (!pickedByUser || Date.now() - pickedAt < 600) return;
        resumeAfterUser();
      }, { passive: true });
    }
  ...
  window.__catCarousel = {
    activeConfig: activeCarouselFor,   // el primero, para quien espere uno solo
    activeList: activeCarousels,       // todos, ya ordenados por posición
    rawList: carouselList,             // sin filtrar, para el panel
    usable: usableCarousel,
